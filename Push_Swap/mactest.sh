...
# Check leaks tool is available
if ! command -v leaks &> /dev/null; then
    echo -e "${RED}Error: 'leaks' tool not found. Install Xcode Command Line Tools:${RESET}"
...
run_test() {
    ...
    check_leaks "$arg"
    if [ $? -eq 0 ]; then LEAK_FOUND=0; else LEAK_FOUND=1; fi
    ...
    echo -e "  $label: leaks=$LK checker=$CH | moves=$MV"
...
for idx in "${!SIZES[@]}"; do
    ...
    for ((i=1; i<=TEST_COUNT; i++)); do
        ...
        echo -e "  Test $i: leaks=$LK checker=$CH | moves=$MV"
